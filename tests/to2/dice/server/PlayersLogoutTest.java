...
public class PlayersLogoutTest {
    ...
    public void testPlayersLogout() throws Exception {
        ...
        assertEquals("Sit down correct player", responses[5].type, Response.Type.SUCCESS);

        responses[6] = server.logout("Janusz");
        assertEquals("Sign out with correct login", responses[6].type, Response.Type.SUCCESS);

        responses[7] = server.logout("Michal");
        assertEquals("Sign out with correct login", responses[7].type, Response.Type.SUCCESS);
    }
}